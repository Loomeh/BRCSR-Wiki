...
:::note
IMPORTANT: The autosplitting and load removing features in SpeedrunUtils require the LiveSplit Server plugin to be installed.
...
The in-game SpeedrunUtils menu should say "Connected to LiveSplit" in bright green text if you've set this up correctly.
:::
...
* **Splits**: Allows you to set what in-game events do/do not auto split. It is set to the any% splits by default. (Note, you can configure splits in-game with the SpeedrunUtils menu, editing this file directly is unnecessary.)

Unfortunately for controller players you cannot bind controller buttons directly for SpeedrunUtils commands. You can however set a single button to a keystroke using Steam Input (pictured below) or if you have the game on GoG you can use a program like [JoyToKey](https://joytokey.net/en/). 
